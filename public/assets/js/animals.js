...
const getAnimals = (formData = {}) => {
  ...
  fetch(queryUrl)
    .then(response => {
      // check to see if the ok property is true or false
      if (!response.ok) {
        // alert the user if there's an error
        return alert('Error: ' + response.statusText);
      }
      // if everything is ok use the .json() method to parse the response into readable json
      return response.json();
    })
    .then(animalData => {
      // send the array of animal data to the printResults() function, 
      // which generates cards for each animal and prints them to the page
